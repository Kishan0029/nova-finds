...
async function main() {
  console.log("Fetching images from Deodap...");
  
  let sql = `-- Add the images array column if it doesn't exist\n`;
  sql += `ALTER TABLE products ADD COLUMN IF NOT EXISTS images text[] DEFAULT '{}';\n\n`;

  for (const p of products) {
    try {
      // 1. Search for the product to get its handle
      ...
      if (results && results.length > 0) {
        ...
        // 2. Fetch the full product JSON from Shopify
        ...
        if (shopifyProduct && shopifyProduct.images && shopifyProduct.images.length > 0) {
          ...
          const primaryImage = shopifyProduct.image?.src || shopifyProduct.images[0].src;
          
          // Grab up to 4 images for the gallery
          const galleryImages = shopifyProduct.images.slice(0, 4).map(img => img.src);
          
          // Format the array for PostgreSQL: ARRAY['url1', 'url2']
          const arrayStr = `ARRAY[${galleryImages.map(url => `'${url}'`).join(', ')}]`;
          
          // Generate UPDATE statement matching our DB name
          sql += `UPDATE products SET image = '${primaryImage}', images = ${arrayStr} WHERE name = '${p.name.replace(/'/g, "''")}';\n`;
          console.log(`✅ Generated SQL for ${p.name}`);
        } else {
          console.log(`⚠️ No images found for ${p.name}`);
        }
      } else {
  ...
  const outputPath = 'e:/08_Miscellanious Projects/Nova Finds/supabase/update_all_images.sql';
